...
const WellBeingSanctuary = () => {
  ...
  const stressReliefContent = (
    <LinearGradient colors={colors.gradient.energy} style={styles.cardGradient}>
      ...
      <Text style={styles.cardSubtitle}>Feeling stressed? Try these mindful solutions:</Text>
      <View style={styles.affirmationContainer}>
        <Text style={styles.affirmationQuote}>"You are stronger than you think. This moment will pass."</Text>
      </View>
      <View style={styles.tipsList}>
        ...
        <View style={styles.tipItem}>
          <Text style={styles.tipBullet}>•</Text>
          <Text style={styles.tipText}>Take 3 deep breaths, slowly and mindfully.</Text>
        </View>
        <View style={styles.tipItem}>
          <Text style={styles.tipBullet}>•</Text>
          <Text style={styles.tipText}>Drink water and do gentle stretches.</Text>
        </View>
      </View>
      <View style={styles.factContainer}>
        <Text style={styles.factText}>
          💡 Stress is a normal response. Taking a mindful pause can help reset your mind and body.
        </Text>
      </View>
    </LinearGradient>
  )
  ...
  const excitedContent = (
    <LinearGradient colors={colors.gradient.energy} style={styles.cardGradient}>
      ...
      <Text style={styles.cardSubtitle}>You're feeling excited! Here are ways to channel that positive energy:</Text>
      <View style={styles.affirmationContainer}>
        <Text style={styles.affirmationQuote}>"Gratitude turns what we have into enough."</Text>
      </View>
      <View style={styles.tipsList}>
        <View style={styles.tipItem}>
          <Text style={styles.tipBullet}>•</Text>
          <Text style={styles.tipText}>Take a moment to celebrate your wins</Text>
        </View>
        <View style={styles.tipItem}>
          <Text style={styles.tipBullet}>•</Text>
          <Text style={styles.tipText}>Share your joy with someone special</Text>
        </View>
        <View style={styles.tipItem}>
          <Text style={styles.tipBullet}>•</Text>
          <Text style={styles.tipText}>Practice gratitude and savor the moment</Text>
        </View>
      </View>
    </LinearGradient>
  )
  ...
  // Enhanced bot response based on mood
  const botResponseByMood = {
    ...
    sad: "It's completely okay to feel sad. Be gentle with yourself today and remember that your feelings are valid. Take things one step at a time.",
    null: "How are you feeling today? I'm here to provide personalized guidance based on your current mood.",
  }
  ...
  const ChatModal = () => (
    ...
    >
      <LinearGradient colors={colors.gradient.focus} style={styles.fullChatModal}>
        <View style={styles.chatHeader}>
          <View style={styles.fenoLogoContainer}>
            <SvgXml xml={fenoLogoXml} width={70} height={38} />
          </View>
          <TouchableOpacity style={styles.fullChatCloseButton} onPress={() => setChatModalVisible(false)}>
          ...
          <View style={styles.fullChatBubbles}>
            <View style={styles.fullChatBubbleBot}>
              <Text style={styles.fullChatText}>
                {selectedSample ? selectedSample.a : botResponseByMood[moodKey] || botResponseByMood.null}
              </Text>
            </View>
            ...
            <View style={styles.fullChatSampleList}>
              {sampleQuestions.map((item, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={[styles.fullChatSampleBubble, selectedSample?.q === item.q && styles.selectedSampleBubble]}
                  onPress={() => setSelectedSample(item)}
                >
                  <Text style={[styles.fullChatSampleText, selectedSample?.q === item.q && styles.selectedSampleText]}>
                    {item.q}
                  </Text>
                </TouchableOpacity>
              ))}
  ...
  useEffect(() => {
    if (isBreathing && !isSessionComplete) {
      if (breathingPhase === "inhale") {
        setDisplayText(inhaleMessages[Math.floor(Math.random() * inhaleMessages.length)])
        phaseTimeoutRef.current = setTimeout(() => setBreathingPhase("exhale"), 4000)
      } else if (breathingPhase === "exhale") {
        setDisplayText(exhaleMessages[Math.floor(Math.random() * exhaleMessages.length)])
        phaseTimeoutRef.current = setTimeout(() => setBreathingPhase("inhale"), 4000)
      }
    }
  ...
  const BreathingModal = () => (
    ...
    >
      <StatusBar backgroundColor={colors.success[600]} barStyle="light-content" />
      <LinearGradient colors={colors.gradient.calm} style={styles.modalContainer}>
        ...
        <View style={styles.modalContent}>
          {!isBreathing && !isSessionComplete && (
            <>
              ...
              <Text style={styles.modalSubtitle}>Find a comfortable position and prepare for a moment of peace.</Text>
              <TouchableOpacity
          ...
          {isBreathing && !isSessionComplete && (
            <>
              <Text style={styles.breathingPhaseText}>{breathingPhase === "inhale" ? "Inhale" : "Exhale"}</Text>
          ...
          {isSessionComplete && (
            <>
              <Text style={styles.congratulatoryMessage}>
                Beautiful work. Your mind and body thank you for this moment of peace.
              </Text>
              <TouchableOpacity style={styles.stopButton} onPress={() => setModalVisible(false)}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Your daily companion for mindful living</Text>
      </View>
      ...
      >
        ...
        {!moodKey && (
          <View style={styles.promptCard}>
            ...
            <Text style={styles.promptText}>
              Let us know how you're feeling today to receive personalized guidance and support.
            </Text>
          </View>
        )}

        {/* Dynamic mood-based content */}
        {moodKey === "stressful" && <View style={styles.card}>{stressReliefContent}</View>}

        {moodKey === "excited" && <View style={styles.card}>{excitedContent}</View>}
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <LinearGradient colors={colors.gradient.focus} style={styles.cardGradient}>
            <View style={styles.chatCardHeader}>
              <View style={styles.chatIconContainer}>
            <Ionicons name="chatbubble-ellipses-outline" size={24} color="#862ec2" />
              </View>
            ...
            <Text style={styles.chatDescription}>
              Get personalized guidance and support based on your current mood and feelings
            </Text>
            <View style={styles.chatCTA}>
              <Text style={styles.chatCTAText}>Tap to start conversation →</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
        ...
        <View style={styles.card}>
          <LinearGradient colors={colors.gradient.wellness} style={styles.cardGradient}>
            <View style={styles.cardHeader}>
              <View style={styles.cardIconContainer}>
          ...
          <LinearGradient colors={colors.gradient.energy} style={styles.cardGradient}>
            ...
            <View style={styles.moodContainer}>
              <Text style={styles.moodText}>
                {loadingMood ? "Loading..." : mood && mood.mood ? mood.mood : "No mood recorded today"}
              </Text>
              <Text style={styles.moodMotivation}>
                {(() => {
                  if (loadingMood) return ""
                  if (!mood || !mood.mood) return "Share your mood to unlock personalized wellness insights!"
                  ...
                  if (m.includes("happy")) return "Your happiness is contagious! Keep shining and sharing your light."
                  if (m.includes("meh"))
            ...
            >
              {!revealed ? (
                <View style={styles.quoteRevealContainer}>
                  ...
                  <Text style={styles.quoteRevealText}>Tap to reveal your daily inspiration</Text>
                </View>
              ) : (
                <Animated.View
                  style={[
                ...
                >
                  <Text style={styles.quoteEmoji}>{QUOTES[quoteIdx]?.emoji}</Text>
                  ...
                  <View style={styles.quoteActions}>
                    ...
                    {revealCount < 2 && (
                      <TouchableOpacity style={styles.showAnotherButton} onPress={showAnother}>
